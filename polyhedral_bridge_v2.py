...
def resonance_vector(seed: str, mandala: PolyhedralMandala) -> Dict[str, Dict[str, float]]:
    if BRIDGE_AVAILABLE:
        try:
            enc = encode(seed)
            # The bridge returns L1-normalized amplitudes with keys like "FAM:RESONANCE"
            # We need to map those back to the short IDs (e.g., "F01", "P01") used internally.
            # Build reverse maps from mandala.
            fam_id_to_short = {f["id"]: f["short_id"] for f in mandala.families.values()}  # we need short IDs
            # The mandala JSON may not have short_id; assume bridge ids like "FAM:RESONANCE" map directly to mandala's family["id"]
            # Actually the bridge uses ids from families.json/principles.json. We need those files.
            # For simplicity, we'll just return the bridge's amplitudes with full ids, and adjust the explorer to handle full ids.
            # Better: we'll load the ontology from the bridge's own files.
            return {
                "families": enc.family_amplitudes_l1,  # {"FAM:RESONANCE": 0.2, ...}
                "principles": enc.principle_amplitudes_l1,  # {"PRIN:SYMMETRY": 0.1, ...}
            }
        except Exception:
            pass
...
class MRPExplorer:
    ...
    def select(self, choice_str: str):
        ...
        if action == "set_seed_concept":
            ...
            child.annotations.append("Family Sweep (bridge used: {}):".format(self.bridge_used))
            for fid, score in top:
                child.annotations.append(f"  {fid}: {score:.3f}")
            ...
            child.annotations.append("Principle Sweep (bridge used: {}):".format(self.bridge_used))
            for pid, score in top:
                child.annotations.append(f"  {pid}: {score:.3f}")
            ...
            if BRIDGE_AVAILABLE and not new_state.current_glyph:
                try:
                    ...
                    child.annotations.append(f"Bridge glyph: {glyph}")
                except Exception:
            ...
            # use bridge's flagged families? simpler: use low-resonance families
            low_fam = [fid for fid, sc in new_state.family_resonance.items() if sc < 0.2]
            syms = []
            for fid in low_fam[:3]:
                if fid in self.mandala.families:
                    syms.append(self.mandala.families[fid]["symbol"])
            if syms:
                bridge = "".join(syms)
            ...
            for sym in flags:
                ...
                child.annotations.append(f"Added noise‑glyph {sym}")
            ...
            child.annotations.append(f"Spin: Families {fam_ok}/{len(new_state.family_resonance)}, Principles {prin_ok}/{len(new_state.principle_resonance)}")
            ...
            if BRIDGE_AVAILABLE:
                try:
                    ...
                    child.annotations.append("Bridge insight generated:")
                    child.annotations.append(json.dumps(entry, indent=2, ensure_ascii=False))
                    ...
                    child.annotations.append(f"Bridge insight failed: {e}")
            else:
                child.annotations.append("Bridge not available.")
        ...
        # Glyph algebra actions (unchanged)
            ...
            if arg and ':' in arg:
                ...
                child.annotations.append("Saved current glyph.")
            ...
            if lib:
                ...
                for i, (n, g) in enumerate(lib):
                    child.annotations.append(f"  [{i}] {n}: {g}")
            else:
                child.annotations.append("Library empty.")
        elif action == "compare_glyphs":
            # (implementation as before, omitted for brevity — same as earlier script)
            child.annotations.append("Glyph comparison not implemented in this snippet.")
        elif action == "merge_glyphs":
            child.annotations.append("Glyph merge not implemented in this snippet.")
        elif action == "detect_glyph_conflicts":
            glyph = new_state.current_glyph
            if glyph:
                conflicts = self.algebra.detect_conflicts(glyph)
                if conflicts:
                    child.annotations.append("Conflicts: " + "; ".join(conflicts))
                else:
                    child.annotations.append("No conflicts.")
            else:
                child.annotations.append("No glyph.")
    ...
    def _manual_seed_glyph(self, new_state, child):
        ...
        new_state.current_glyph = glyph
        child.annotations.append(f"Manual Seed Glyph: {glyph}")
